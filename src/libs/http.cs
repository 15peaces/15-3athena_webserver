/***********************************************
 *
 *     15-3athena - Webserver       
 *   Copyright © 2025 15peaces
 *
 ***********************************************
 * 
 * 
...
namespace _15WebServ
{
    class HttpHandler:WebServ
    {
        class c_json
        {
            ...
            public c_json_data data { get; set; }
        }
        ...
        public void HandleRequest(NetworkStream ns, string request)
        {
            ...
            switch (split_request[0]) // Parse command
            {
                case "POST":
                    ...
                    {
                        case "/userconfig/load":
                            string json;

                            if (!GetValueFromRequest("CONTENT-TYPE").Contains("multipart/form-data; boundary="))
                            {
                                console.warning("Wrong content type for post request.");
                                break;
                            }

                            if (GetValueFromRequest("EXPECT") == "100-continue")
                            {
                                buffer = Encoding.ASCII.GetBytes(httpver_str[(int)ver] + " 100 Continue\n\r");
                                _SendBuffer();
                            }

                            var json_cls = new c_json
                            {
                                Type = 1,
                                data = null
                            };

                            json = JsonConvert.SerializeObject(json_cls);

                            res_str += httpver_str[(int)ver] + " 200 OK\n\r"+json;
                            buffer = Encoding.ASCII.GetBytes(res_str);
                            console.debug("Answering: " + res_str);
                            _SendBuffer();
                            return;
                        default:
